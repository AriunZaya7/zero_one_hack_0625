...
# ══════════════════════════════════════════════════════════════════════════════
# FULL-LENGTH PROBE SEQUENCES — for models that need global context (XGBoost)
# ══════════════════════════════════════════════════════════════════════════════
# These are realistic ~50-step sequences with violations injected at realistic
# positions, giving XGBoost enough history to activate its global features.

# Shared backbone used to build full sequences
_BACKBONE_START = [
    "RECEIVE WAFER LOT", "LOT IDENTIFICATION", "PRE CLEAN INSPECTION",
    "MEASURE INITIAL THICKNESS", "MEASURE SURFACE PLANARITY",
]
_BACKBONE_LITHO1 = [
    "HF DIP", "DRY WAFER", "THERMAL OXIDATION", "MEASURE OXIDE THICKNESS",
    "COAT PHOTORESIST", "SOFT BAKE", "ALIGN MASK LEVEL 1",
    "EXPOSE LITHO LEVEL 1", "DEVELOP PHOTORESIST",
]
_BACKBONE_ETCH1 = [
    "OXIDE ETCH", "STRIP RESIST", "CLEAN AFTER ETCH",
]
_BACKBONE_IMPLANT = [
    "HF DIP", "DRY WAFER", "DEPOSIT POLYSILICON", "MEASURE THICKNESS",
    "COAT PHOTORESIST", "SOFT BAKE", "ALIGN MASK LEVEL 2",
    "EXPOSE LITHO LEVEL 2", "DEVELOP PHOTORESIST",
    "OXIDE ETCH", "IMPLANT P BODY", "STRIP RESIST", "ANNEAL",
...
_BACKBONE_ILD = [
    "HF DIP", "DRY WAFER", "DEPOSIT INTERLAYER DIELECTRIC",
    "MEASURE ILD THICKNESS", "CMP DIELECTRIC", "MEASURE POST CMP",
    "VIA ETCH", "CLEAN AFTER ETCH", "FILL VIA TUNGSTEN",
]
_BACKBONE_METAL = [
    "HF DIP", "DRY WAFER", "DEPOSIT METAL 1", "MEASURE METAL THICKNESS",
    "COAT PHOTORESIST", "SOFT BAKE", "ALIGN MASK LEVEL 3",
    "EXPOSE LITHO LEVEL 3", "DEVELOP PHOTORESIST",
    "METAL ETCH", "STRIP RESIST", "CLEAN AFTER ETCH",
]
_BACKBONE_PASSIVATION = [
    "HF DIP", "DRY WAFER", "DEPOSIT PASSIVATION", "CURE PASSIVATION",
]
_BACKBONE_END = [
    "COAT PHOTORESIST", "SOFT BAKE", "ALIGN MASK LEVEL 4",
    "EXPOSE LITHO LEVEL 4", "DEVELOP PHOTORESIST",
    "OPEN PAD WINDOW", "STRIP RESIST",
    "PARAMETRIC TEST", "WAFER SORT TEST", "FINAL INSPECTION", "SHIP LOT",
]

def _full_valid_seq() -> list[str]:
    """Build a complete valid IGBT-like sequence ~80 steps."""
    return (
        _BACKBONE_START +
        _BACKBONE_LITHO1 +
        _BACKBONE_ETCH1 +
...
def _inject_violation(full_seq: list[str], find_step: str,
                       replace_with: list[str],
                       remove_steps: list[str] = None) -> list[str]:
    """
    Build an invalid sequence by modifying the full valid sequence.
    find_step:    step name to find as injection point
    replace_with: steps to insert before find_step
    remove_steps: steps to remove from the sequence
    """
    seq = list(full_seq)
    if remove_steps:
        seq = [s for s in seq if s not in remove_steps]
    if find_step and replace_with:
        idx = next((i for i, s in enumerate(seq) if s == find_step), -1)
        if idx >= 0:
            seq = seq[:idx] + replace_with + seq[idx:]
    return seq


# Full-length probe pairs — one per rule
RULE_PROBES_FULL = []

_valid = _full_valid_seq()

# RULE 1: DEP_NO_CLEAN — remove HF DIP + DRY WAFER before THERMAL OXIDATION
_invalid_dep = [s for s in _valid
                if not (s in ("HF DIP", "DRY WAFER") and
                        _valid.index(s) < _valid.index("THERMAL OXIDATION") + 3)]
RULE_PROBES_FULL.append({
    "rule": "RULE_DEP_NO_CLEAN",
    "description": "Deposition must always be preceded by a cleaning step",
    "violation": "THERMAL OXIDATION without HF DIP + DRY WAFER before it",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_dep,
})

# RULE 2: METAL_ETCH_NO_LITHO — remove EXPOSE + DEVELOP before METAL ETCH
_invalid_metal = [s for s in _valid
                  if s not in ("EXPOSE LITHO LEVEL 3", "DEVELOP PHOTORESIST")
                  or _valid.index(s) < _valid.index("METAL ETCH") - 5]
RULE_PROBES_FULL.append({
    "rule": "RULE_METAL_ETCH_NO_LITHO",
    "description": "Lithography must precede metal etching",
    "violation": "METAL ETCH without EXPOSE LITHO LEVEL 3 and DEVELOP PHOTORESIST",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_metal,
})

# RULE 3: ETCH_NO_MASK — remove DEVELOP PHOTORESIST before OXIDE ETCH
_invalid_etch = list(_valid)
try:
    dev_idx = _invalid_etch.index("DEVELOP PHOTORESIST")
    etch_idx = _invalid_etch.index("OXIDE ETCH")
    if dev_idx < etch_idx:
        _invalid_etch.pop(dev_idx)
except ValueError:
    pass
RULE_PROBES_FULL.append({
    "rule": "RULE_ETCH_NO_MASK",
    "description": "A developed mask must exist before any etch",
    "violation": "OXIDE ETCH without DEVELOP PHOTORESIST immediately before it",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_etch,
})

# RULE 4: LITHO_LEVEL_SKIP — replace EXPOSE LITHO LEVEL 2 with LEVEL 3
_invalid_skip = [
    "EXPOSE LITHO LEVEL 3" if s == "EXPOSE LITHO LEVEL 2" else s
    for s in _valid
]
RULE_PROBES_FULL.append({
    "rule": "RULE_LITHO_LEVEL_SKIP",
    "description": "Lithography levels must increment sequentially",
    "violation": "Sequence jumps from LITHO LEVEL 1 to LITHO LEVEL 3 skipping 2",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_skip,
})

# RULE 5: IMPLANT_NO_MASK — remove OXIDE ETCH before IMPLANT P BODY
_invalid_implant = list(_valid)
try:
    oe_idx  = next(i for i in range(len(_invalid_implant))
                   if _invalid_implant[i] == "OXIDE ETCH"
                   and i < _invalid_implant.index("IMPLANT P BODY"))
    _invalid_implant.pop(oe_idx)
...
RULE_PROBES_FULL.append({
    ...
    "violation": "IMPLANT P BODY without prior OXIDE ETCH to open window",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_implant,
})

# RULE 6: CMP_NO_DEP — remove DEPOSIT INTERLAYER DIELECTRIC before CMP
_invalid_cmp = [s for s in _valid if s != "DEPOSIT INTERLAYER DIELECTRIC"]
RULE_PROBES_FULL.append({
    "rule": "RULE_CMP_NO_DEP",
    "description": "CMP planarization requires a prior deposition step",
    "violation": "CMP DIELECTRIC without DEPOSIT INTERLAYER DIELECTRIC",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_cmp,
})

# RULE 7: PAD_OPEN_BEFORE_DEP — move OPEN PAD WINDOW before passivation block
_invalid_pad = list(_valid)
try:
    pad_idx  = _invalid_pad.index("OPEN PAD WINDOW")
    dep_idx  = _invalid_pad.index("DEPOSIT PASSIVATION")
    if pad_idx > dep_idx:
        _invalid_pad.pop(pad_idx)
        _invalid_pad.insert(dep_idx, "OPEN PAD WINDOW")
except ValueError:
    pass
RULE_PROBES_FULL.append({
    "rule": "RULE_PAD_OPEN_BEFORE_DEP",
    "description": "Bond pad window only after passivation deposited and cured",
    "violation": "OPEN PAD WINDOW moved before DEPOSIT PASSIVATION",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_pad,
})

# RULE 8: TEST_BEFORE_PASSIVATION — move PARAMETRIC TEST before CURE PASSIVATION
_invalid_test = list(_valid)
try:
    pt_idx   = _invalid_test.index("PARAMETRIC TEST")
    cure_idx = _invalid_test.index("CURE PASSIVATION")
    if pt_idx > cure_idx:
        _invalid_test.pop(pt_idx)
        _invalid_test.insert(cure_idx, "PARAMETRIC TEST")
except ValueError:
    pass
RULE_PROBES_FULL.append({
    "rule": "RULE_TEST_BEFORE_PASSIVATION",
    "description": "Electrical tests must come after passivation is cured",
    "violation": "PARAMETRIC TEST moved before CURE PASSIVATION",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_test,
})

# RULE 9: SHIP_BEFORE_TEST — swap SHIP LOT and WAFER SORT TEST
_invalid_ship = list(_valid)
try:
    ship_idx = _invalid_ship.index("SHIP LOT")
    test_idx = _invalid_ship.index("WAFER SORT TEST")
    if ship_idx > test_idx:
        _invalid_ship[ship_idx], _invalid_ship[test_idx] = (
            _invalid_ship[test_idx], _invalid_ship[ship_idx])
except ValueError:
    pass
RULE_PROBES_FULL.append({
    "rule": "RULE_SHIP_BEFORE_TEST",
    "description": "Wafer sort test must complete before shipping",
    "violation": "SHIP LOT swapped to appear before WAFER SORT TEST",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_ship,
})

# RULE 10: BACKSIDE_BEFORE_PASSIVATION — move backside block before passivation
_invalid_back = (
    _BACKBONE_START + _BACKBONE_LITHO1 + _BACKBONE_ETCH1 +
    _BACKBONE_IMPLANT + _BACKBONE_ILD + _BACKBONE_METAL +
    ["BACKSIDE GRIND", "BACKSIDE CLEAN", "DEPOSIT BACKSIDE METAL"] +
    _BACKBONE_PASSIVATION +
    [s for s in _BACKBONE_END if "BACKSIDE" not in s]
)
RULE_PROBES_FULL.append({
    "rule": "RULE_BACKSIDE_BEFORE_PASSIVATION",
    "description": "Backside metal only after frontside passivation is cured",
    "violation": "Entire backside block moved before DEPOSIT PASSIVATION",
    "valid_seq":   _valid,
    "invalid_seq": _invalid_back,
})
...
def run_probes(adapter: BaseModelAdapter, use_full_sequences: bool = False) -> dict:
    """
    Run all 10 rule probes on any model adapter.

    use_full_sequences: if True, uses full-length (~80 step) sequences
                        with violations injected at realistic positions.
                        Recommended for XGBoost and other models that need
                        global context to activate their features.
                        Default False uses short 5-9 step probe sequences
                        which work well for n-gram.
    """
    probes = RULE_PROBES_FULL if use_full_sequences else RULE_PROBES
    probe_mode = "full-length" if use_full_sequences else "short"

    results = []
    print(f"\nRunning {len(probes)} rule probes on: {adapter.model_name}")
    print(f"Probe mode: {probe_mode} sequences")
    if use_full_sequences:
        print(f"  Full sequence length: ~{len(_valid)} steps")
        print(f"  Violations injected at realistic positions in full sequences")
    print(f"{'─'*80}")
    ...
    for probe in probes:
        ...
        results.append({
            ...
            "valid_seq":   probe["valid_seq"][:10],   # truncate for JSON readability
            "invalid_seq": probe["invalid_seq"][:10],
...
def main():
    ...
    parser.add_argument("--full_sequences",
                        action="store_true",
                        help="Use full-length (~80 step) probe sequences (auto for xgboost)")
    parser.add_argument("--short_sequences",
                        action="store_true",
                        help="Force short probe sequences even for xgboost")
    ...
    # ── Run probes ─────────────────────────────────────────────────────────────
    # Auto-use full sequences for external models (XGBoost etc.)
    # Can be overridden with --full_sequences or --short_sequences flags
    use_full = args.full_sequences or (args.model == "xgboost")
    if args.short_sequences:
        use_full = False
    probe_data = run_probes(adapter, use_full_sequences=use_full)
