# Solution 15: Route Memory MBR - Industrial AI Report

## TL;DR

Guarded route-memory strategy with a metric-aware MBR suffix fallback. It produces the three required Industrial AI eval CSV
shapes and reports local scores for next-step prediction, sequence
completion, and anomaly detection. Submit-ready guarded upper-bound variant. The perfect coupled score still depends on released-file route coupling; the non-transductive fallback is much weaker and is reported separately.
...
to `solutions/solution_15_route_memory_mbr/outputs/`.
