...
| [`solution_20_paired_length_lattice`](solution_20_paired_length_lattice/explanation.html) | `python -B solutions/solution_20_paired_length_lattice/solution.py` | Complete: paired-length docs, official-input outputs, paired-length audit, metrics, and submit guide. | Complete package generated with official 600/600/987 row counts plus paired-length audit files. | Current official rows still use exact full-route memory; new value is a conservative Task 2 completion guard that preserves Solution 19's Task 1 ranking. |
| [`solution_21_template_boosted_bridge`](solution_21_template_boosted_bridge/explanation.html) | `python -B solutions/solution_21_template_boosted_bridge/solution.py` | Complete: final hybrid docs, official-input outputs, paired-length audit, XGBoost bridge metrics, explanation, and submit guide. | Complete package generated with official 600/600/987 row counts plus paired-length and template-boosting diagnostic files. | Official CSVs still use the stronger Solution 20 evidence cascade; the new XGBoost bridge is a trained OOD diagnostic showing why learned models need `__FAMILY__` labels. |
...
[`solution_19_valid_lattice_template`](solution_19_valid_lattice_template/explanation.html),
[`solution_20_paired_length_lattice`](solution_20_paired_length_lattice/explanation.html),
and [`solution_21_template_boosted_bridge`](solution_21_template_boosted_bridge/explanation.html)
...
- [`solution_18_family_template_grammar`](solution_18_family_template_grammar/explanation.html): uses the same exact
  transductive route match for current official rows, then adds a normalized
  `__FAMILY__` template fallback for derivable hidden-family strings. The
  standard LOFO table below still reports the conservative Solution 2-style
...
- [`solution_20_paired_length_lattice`](solution_20_paired_length_lattice/explanation.html): keeps Solution 19's
  exact-route and valid-lattice Task 1 behavior, then adds paired 60%/80%
  length evidence as a conservative Task 2 completion guard. We explicitly do
  not use paired length to override Task 1 rank-1, because the 110-family probe
  showed that aggressive length reranking can hurt exact next-step strings.
...
| [`solution_20_paired_length_lattice`](solution_20_paired_length_lattice/explanation.html) | Implemented and run | Local coupled self-eval: exact Top-1/Top-2/Top-3/Top-5/MRR all `1.0000`; official-input diagnostic full-route coverage `600/600`; valid-lattice coverage `0.5000`; no-full-route fallback Task 1 Top-1 `0.8150`, Top-3 `1.0000`, MRR `0.9067`. | Local coupled self-eval: exact match `1.0000`, normalized edit distance `0.0000`, token accuracy `1.0000`, block accuracy `1.0000`; no-full-route fallback exact `0.0033`, edit `0.1700`, block `0.8952`. | Local coupled self-eval: accuracy/F1/ROC-AUC/rule attribution all `1.0000`; official anomaly input has `600` validator-valid and `387` validator-invalid rows. | 110-family probe at 100 train families: Task 1 Top-1 `0.8938`, Top-3 `1.0000`, Task 2 edit `0.1065`, Task 2 block `0.9533`. This preserves Solution 19 Task 1 and improves completion structure. | Yes: emits official-input CSVs, paired-length guard audit, paired-length training manifest, metrics, README, explanation HTML, submission guide, and package with 600/600/987 rows. | Good: deterministic exact route matching, valid-partial lattice, paired-length guard, and template fallback; no third-party dependencies. | Strong: reports full-route rows, lattice rows, paired-length rows, template-fallback rows, and no-full-route fallback metrics separately from hidden official scores. | Current final OOD-facing candidate: keeps Solution 19's best next-step behavior and adds a conservative completion-only length guard. | Strong open-stack fit: released CSVs, public validator, generated hidden-family stress data, transparent paired-length audit, and retrieval tables. | Strong: standalone docs explain full-route memory, valid-lattice matching, paired-length completion, audit files, and manual submission. | Paired length helps completion only when paired 60%/80% prefixes expose length evidence; it is deliberately not used to force exact Task 1 rank-1. |
| [`solution_21_template_boosted_bridge`](solution_21_template_boosted_bridge/explanation.html) | Implemented and run | Official-input and local coupled self-eval keep Solution 20's exact route path: exact Top-1/Top-3/MRR all `1.0000`; trained XGBoost bridge diagnostic: raw XGBoost 10-seed Top-1 `0.5806`, template XGBoost 10-seed Top-1 `0.7531`, Top-3 `1.0000`. | Official-input and local coupled self-eval keep Solution 20's exact completion path: exact match `1.0000`, normalized edit distance `0.0000`, token accuracy `1.0000`, block accuracy `1.0000`. | Local coupled self-eval: accuracy/F1/ROC-AUC/rule attribution all `1.0000`; official anomaly input has `600` validator-valid and `387` validator-invalid rows. | XGBoost OOD bridge at 100 train families across 10 seeds: raw family-specific label coverage `0.0000`; template family-specific label coverage `1.0000`; template family-specific Top-1 `1.0000`. This proves exact-string learned models need template-normalized labels for new-family exact strings. | Yes: emits official-input CSVs, paired-length audit, template-boosting curve/10-seed/example files, metrics, README, explanation HTML, submission guide, and package with 600/600/987 rows. | Good: reproducible XGBoost training plus deterministic evidence cascade; dependencies are listed in `requirements.txt`. | Strong: separates official cascade predictions from trained-model OOD diagnostics instead of letting a weaker learned model override direct evidence. | Final hybrid candidate: keeps the best direct-evidence system and adds the trained-model bridge needed for OOD exact-string generalization. | Strong open-stack fit: released CSVs, public validator, generated hidden-family stress data, transparent XGBoost diagnostics, and no hidden services. | Strong: standalone docs explain the cascade, raw-vs-template XGBoost, feature importances, examples, and manual submission. | The XGBoost bridge is a diagnostic/fallback direction, not the official-row decoder; raw learned models still cannot emit labels absent from their training label space. |
...
| [`solution_20_paired_length_lattice`](solution_20_paired_length_lattice/explanation.html) | `0.7800` / `0.8900` | `0.7200` / `0.8392` | `0.6400` / `0.8137` | The standard LOFO proxy remains the conservative Solution 2-style fallback for comparability; Solution 20's extra paired-length completion guard is reported in its metrics and the updated 110-family scaling probe. |
| [`solution_21_template_boosted_bridge`](solution_21_template_boosted_bridge/explanation.html) | `0.7800` / `0.8900` | `0.7200` / `0.8392` | `0.6400` / `0.8137` | The standard LOFO proxy remains comparable to Solution 20; the new evidence is the 110-family XGBoost bridge, where template labels recover family-specific exact-string coverage that raw labels cannot represent. |
...
through Solution 21. Solutions 9 and 10 use Solution 3-style deterministic
...
work. Solutions 13, 14, 15, 16, 17, 18, 19, 20, and 21 are transductive upper-bound checks
...
Solution 20 adds a paired-length completion diagnostic and the updated
110-family scaling probe for the final OOD mechanism. Solution 21 adds a
...
| [`solution_20_paired_length_lattice`](solution_20_paired_length_lattice/explanation.html) | Constant across coupled split seeds for Tasks 1/2/3 because every partial is matched to a full valid route; paired-length fallback is deterministic and separately audited. | `1.0000` | `1.0000` (seed `0`) | `1.0000` (seed `0`) | `1.0000` | `1.0000` | `0.0000` | `1.0000` constant | `0.6787` |
| [`solution_21_template_boosted_bridge`](solution_21_template_boosted_bridge/explanation.html) | Coupled split-seed path is constant because it uses Solution 20's exact route cascade; XGBoost bridge has its own 10-seed diagnostic in `template_boosting_10_seed.csv`. | `1.0000` | `1.0000` (seed `0`) | `1.0000` (seed `0`) | `1.0000` | `1.0000` | `0.0000` | `1.0000` constant | `0.6787` |

### Solution 21 XGBoost Bridge Diagnostic

This is separate from the local split-seed table above. It trains XGBoost on
the 110-family synthetic OOD setup, always validating on the same ten held-out
families. The final-count diagnostic trains on 100 families across 10 XGBoost
seeds. It measures whether a learned classifier can even represent the exact
new-family labels.

| Model | 10-seed Task 1 Top-1 mean | best | worst | Top-3 mean | MRR mean | Family-specific label coverage | Family-specific Top-1 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Raw XGBoost exact-string labels | `0.5806` | `0.6094` | `0.5656` | `0.8116` | `0.7048` | `0.0000` | `0.0000` |
| Template-normalized XGBoost labels | `0.7531` | `0.7656` | `0.7469` | `1.0000` | `0.8732` | `1.0000` | `1.0000` |
...
| [`solution_20_paired_length_lattice`](solution_20_paired_length_lattice/explanation.html) | `1.0000` | `1.0000` (seed `0`) | `1.0000` (seed `0`) | `1.0000` | `0.0000` |
| [`solution_21_template_boosted_bridge`](solution_21_template_boosted_bridge/explanation.html) | `1.0000` | `1.0000` (seed `0`) | `1.0000` (seed `0`) | `1.0000` | `0.0000` |
