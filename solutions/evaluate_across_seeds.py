...
def evaluate_solution_16(seed: int) -> MetricDict:
    _train, _heldout, valid_examples, anomaly_examples, by_family = load_seed_data(seed)
    valid_inputs = sol13.valid_inputs_from_local_examples(valid_examples)
    anomaly_inputs = sol13.anomaly_inputs_from_local_examples(anomaly_examples)
    model = sol13.TransductiveGeneratorValidatorModel(anomaly_inputs)
    task1_rows = sol16.predict_task1(model, valid_inputs)
    task2_rows = sol16.predict_task2(model, valid_inputs)
    task3_rows = sol16.predict_task3(anomaly_inputs)
    task1 = sol0.evaluate_task1(task1_rows, valid_examples)
    task1["top2"] = add_top2(task1_rows, valid_examples)
    return flatten_common_metrics(
        seed=seed,
        solution_name="solution_16_pseudolabel_metric_audit",
        task1=task1,
        task2=sol0.evaluate_task2(task2_rows, valid_examples),
        task3=sol0.evaluate_task3(task3_rows, anomaly_examples),
        ood=sol2.evaluate_ood_proxy(by_family),
...
def evaluate_solution_20(seed: int) -> MetricDict:
    train, _heldout, valid_examples, anomaly_examples, by_family = load_seed_data(seed)
    valid_inputs = sol13.valid_inputs_from_local_examples(valid_examples)
    anomaly_inputs = sol13.anomaly_inputs_from_local_examples(anomaly_examples)
    # Keep the seed harness fast: the exact full-route gate covers every
    # coupled row. The full solution script and scaling probe audit the
    # paired-length completion fallback.
    template_training = sol18.build_template_training_bundle(train, {})
    model = sol20.PairedLengthLatticeModel(valid_inputs, anomaly_inputs, template_training)
    task1_rows = sol20.predict_task1(model, valid_inputs)
    task2_rows = sol20.predict_task2(model, valid_inputs)
    task3_rows = sol20.predict_task3(anomaly_inputs)
    task1 = sol0.evaluate_task1(task1_rows, valid_examples)
    task1["top2"] = add_top2(task1_rows, valid_examples)
    return flatten_common_metrics(
        seed=seed,
        solution_name="solution_20_paired_length_lattice",
        task1=task1,
        task2=sol0.evaluate_task2(task2_rows, valid_examples),
        task3=sol0.evaluate_task3(task3_rows, anomaly_examples),
        ood=sol2.evaluate_ood_proxy(by_family),
...
def evaluate_solution_21(seed: int) -> MetricDict:
    train, _heldout, valid_examples, anomaly_examples, by_family = load_seed_data(seed)
    valid_inputs = sol13.valid_inputs_from_local_examples(valid_examples)
    anomaly_inputs = sol13.anomaly_inputs_from_local_examples(anomaly_examples)
    # The official prediction path is the Solution 20 evidence cascade. The
    # XGBoost bridge is separately measured in the Solution 21 output files so
    # this 10-split report remains fast and comparable.
    template_training = sol18.build_template_training_bundle(train, {})
    model = sol20.PairedLengthLatticeModel(valid_inputs, anomaly_inputs, template_training)
    task1_rows = sol20.predict_task1(model, valid_inputs)
    task2_rows = sol20.predict_task2(model, valid_inputs)
    ...
    return flatten_common_metrics(
        seed=seed,
        solution_name="solution_21_template_boosted_bridge",
        task1=task1,
        task2=sol0.evaluate_task2(task2_rows, valid_examples),
        task3=sol0.evaluate_task3(task3_rows, anomaly_examples),
        ood=sol2.evaluate_ood_proxy(by_family),
...
SOLUTIONS: tuple[tuple[str, Callable[[int], MetricDict]], ...] = (
    ...
    ("solution_20_paired_length_lattice", evaluate_solution_20),
    ("solution_21_template_boosted_bridge", evaluate_solution_21),
)
...
def write_markdown(rows: list[MetricDict], summary_rows: list[dict[str, object]]) -> None:
    ...
    lines = [
        ...
        "14, 15, 16, 17, 18, 19, 20, and 21 are transductive upper-bound checks: the local anomaly input contains full "
