#!/usr/bin/env python3
"""Solution 5: tuned retrieval + multi-n-gram rank ensemble.

This attempt keeps the same data surface as Solution 1, but changes Task 1
scoring. Instead of one trigram fallback weight, it combines retrieval votes
with n-gram models of several orders using fixed tuned weights.
...
class TunedRankEnsembleModel:
    def __init__(self, train_sequences: dict[str, list[str]]) -> None:
        self.retriever = sol1.HybridRetrievalModel(train_sequences)
        self.ngrams = {
            order: NGramModel(n=order, alpha=0.4).fit(train_sequences.values())
    ...
    def next_step_ranking(
        ...
        for rank, hit in enumerate(self.retriever.retrieve(prefix, family, completion_fraction)):
            if hit.position >= len(hit.sequence):
                continue
            step = hit.sequence[hit.position]
            scores[step] += RETRIEVAL_WEIGHT * hit.score / (1.0 + rank * RETRIEVAL_RANK_DECAY)

        for model in self.ngrams.values():
            for rank, step in enumerate(model.next_step_ranking(prefix, k=20)):
    ...
    def complete(
        ...
        return self.retriever.complete(prefix, family, completion_fraction)


def predict_task1(
    model: TunedRankEnsembleModel,
...
def evaluate_ood_proxy(
    ...
    for family in base.FAMILIES:
        ...
        model = TunedRankEnsembleModel(train)
...
def write_metrics(metrics: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "## Method",
        "",
        f"- N-gram orders: {', '.join(str(order) for order in NGRAM_ORDERS)}",
        f"- Retrieval weight: {RETRIEVAL_WEIGHT}",
        f"- Retrieval rank decay: {RETRIEVAL_RANK_DECAY}",
        f"- N-gram weight: {NGRAM_WEIGHT}",
        "",
        "## Task 1",
        "",
...
def main() -> None:
    ...
    metrics = {
        "solution": "solution_5_tuned_rank_ensemble",
        "seed": SEED,
        "method": {
            "name": "hybrid retrieval + tuned multi-n-gram rank ensemble",
