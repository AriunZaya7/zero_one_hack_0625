"""
generate_kremsians.py — Synthetic 4th semiconductor family for OOD evaluation.

KREMSIANS is a hypothetical power device (a Kremsians-type Gate device) combining
MOSFET and IGBT characteristics. It uses only existing vocabulary steps but creates
novel transition sequences that none of the three training families contain:

  Novel characteristics:
    - Prep: IGBT-style epitaxial wafer check FOLLOWED BY MOSFET-style substrate prep
      → no training family has this exact combination
    - 5 litho levels (MOSFET=4, IGBT=6 — sits between them)
    - Cycle 3: POLYSILICON ETCH → IMPLANT SOURCE DRAIN → IMPLANT N BUFFER (dual implant,
      unique: MOSFET has SOURCE DRAIN only; IGBT has N BUFFER in a separate litho cycle)
    - Includes ANISOTROPIC ETCH SPACER (MOSFET-only) AND DEPOSIT FIELD OXIDE (IGBT-only)
      in the same family — never seen together in training
...
def _cycle3_poly_dual_implant(rng):
    """Poly gate + SOURCE DRAIN + N BUFFER in same cycle — KREMSIANS-unique (litho level 3).

    MOSFET:  poly etch → SOURCE DRAIN only
    IGBT:    poly etch → CHANNEL STOP only; N BUFFER is a separate litho cycle
    KREMSIANS: poly etch → SOURCE DRAIN → N BUFFER (dual implant, novel transition)
    """
    ...
    # DUAL IMPLANT — the novel KREMSIANS signature
...
def _cycle4_field_via(rng):
    ...
    if rng.random() < 0.4:
        s += [_choice(rng, "WET CLEAN RCA1", "RCA CLEAN 1"),
              _choice(rng, "WET CLEAN RCA2", "RCA CLEAN 2"), "HF DIP"]
    s.append("DEPOSIT FIELD OXIDE")           # IGBT step in KREMSIANS field position
...
def _test_suite(rng):
    s = [_choice(rng, "PARAMETRIC TEST", "ELECTRICAL PARAMETRIC TEST"),
         "LEAKAGE TEST",
         "THRESHOLD VOLTAGE TEST",   # KREMSIANS is gate-based like MOSFET
         "SWITCHING TEST",
         "WAFER SORT TEST",
         "YIELD ANALYSIS"]
    return s
...
def generate_kremsians_sequence(rng) -> list[str]:
    seq = []
    seq += _prefix(rng)
    seq += _initial_measurements(rng)
    seq += _pre_process_clean(rng)
    seq += _kremsians_prep(rng)
...
def generate(count: int, seed: int = 42, max_retries: int = 20) -> list[list[str]]:
    ...
    while len(sequences) < count:
        attempts += 1
        seq = generate_kremsians_sequence(rng)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out",      default="training_data/KREMSIANS_variants.csv")
    ...
    if args.validate:
        ...
        print(f"Generating {args.count} KREMSIANS sequences (seed={args.seed})...")
